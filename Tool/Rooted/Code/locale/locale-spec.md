...
## 12. Codebase alignment — web client & parity (April 2026)

### Web (Next.js) user stories

- **As a web user**, I can browse featured locales under `web/app/(dashboard)/locale/` and open a detail screen at `web/app/(dashboard)/locale/[id]/page.tsx` using the locale document id, sharing the same backend `GET /api/v1/locales` and related APIs as mobile.
- **As a mobile user**, I still open locale detail via the TripScore-style route `frontend/app/tripscore/countries/[country]/locations/[location].tsx` with route params (and optional query metadata) as described in §2.

### Product note

Mobile and web use **different route shapes** for locale detail (slug params vs `[id]`). Acceptance criteria for new work should specify both clients or introduce a shared deep-link strategy.

---

**Document Version**: 1.1  
**Last Updated**: April 2026 (§12 web parity)  
