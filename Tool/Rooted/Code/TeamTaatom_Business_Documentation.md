...
11. [Product Specifications Repository](#product-specifications-repository)
12. [App Owner Guide](#app-owner-guide)

---

## 🎯 Executive Summary

**TeamTaatom** is a travel-focused social media platform that combines photo/video sharing with comprehensive location tracking and mapping features. Built with React Native (Expo), an optional Next.js web client, Express.js, and a SuperAdmin panel, it's designed for travelers who want to document their journeys, discover new places, and connect with fellow travelers through location-based content.
...
- **Technology**: React Native (Expo), Next.js (`web/`), Express.js, SuperAdmin (React/Vite)
...
- **Navigation**: Expo Router (file-based routing under `frontend/app/`)
...
- **Role**: Browser-based client aligned with the same backend APIs (feed, locale discovery, shorts, profile, TripScore, collections, chat, settings)
- **Not yet covered in detail**: The per-tab specs under `Tool/Rooted/Code/` describe the mobile app file paths first; web routes live under `web/app/` (for example dashboard feed at `web/app/(dashboard)/feed/page.tsx`, locale browsing at `web/app/(dashboard)/locale/`). Product behavior should match the mobile specs unless intentionally differentiated.
...
- **Root**: `superAdmin/`
- **Purpose**: Internal dashboard for users, travel content, locales, songs, analytics, TripScore analytics, moderation, reports, feature flags, and system settings (see `Tool/Rooted/Code/admin/README.md`).

### Key Technical Features
- **Cross-Platform**: Mobile (iOS and Android) via Expo; additional web client under `web/`
...
Authoritative, menu-level product and technical behavior for the platform is maintained next to this document under `Tool/Rooted/Code/`. Each area has a dedicated markdown spec (flows, APIs, data models, edge cases, and SuperAdmin touchpoints). Use these when scoping features, onboarding, or reviewing parity across mobile, web, and admin.

### Mobile app (primary specs)

| Area | Specification | Primary implementation (mobile) |
|------|----------------|----------------------------------|
| Home feed | `Tool/Rooted/Code/home/home-spec.md` | `frontend/app/(tabs)/home.tsx` |
| Locale discovery & saved places | `Tool/Rooted/Code/locale/locale-spec.md` | `frontend/app/(tabs)/locale.tsx`, `frontend/app/tripscore/countries/[country]/locations/[location].tsx` |
| Create post / short | `Tool/Rooted/Code/post/post-spec.md` | `frontend/app/(tabs)/post.tsx` |
| Profile, settings entry | `Tool/Rooted/Code/profile/profile-spec.md` | `frontend/app/(tabs)/profile.tsx`, `frontend/app/profile/[id].tsx` |
| Shorts feed | `Tool/Rooted/Code/shorts/shorts-spec.md` | `frontend/app/(tabs)/shorts.tsx` |

### SuperAdmin panel

All SuperAdmin menus are specified under `Tool/Rooted/Code/admin/`. See `Tool/Rooted/Code/admin/README.md` for the full menu list (Dashboard, Auth, Users, Travel Content, Songs, Locales, Analytics, TripScore Analytics, Query Monitor, Reports, Moderators, Logs, Feature Flags, Settings). Implementation pages live under `superAdmin/src/pages/` and API routes under `backend/src/routes/enhancedSuperAdminRoutes.js`.

### Web client (`web/`)

The Next.js app mirrors core journeys (feed, discover, locale, shorts, profile, create, collections, activity, chat, settings). Detailed screen-by-screen documentation for web-specific routing is not duplicated in every spec today; treat mobile specs as the behavioral baseline and extend web-only UX in this repository as needed.
...
### Documentation
- **Technical Documentation**: `Tool/Rooted/Code/TeamTaatom_Development_Guide.md`
- **Product specifications (per menu)**: `Tool/Rooted/Code/` (`home/`, `locale/`, `post/`, `profile/`, `shorts/`, `admin/`)
...
**Last Updated**: April 2026 (Product specs index, web & SuperAdmin architecture notes)  
**Version**: 1.5.0  
