...
## 12. Codebase alignment — ads, deep links & playback (April 2026)

### Additional user stories

- **As a user opening Shorts from a profile or saved item**, I can land on a specific reel via **`shortId`** (query param or `initialShortId` prop); the list scrolls to that item when data is ready.
- **As a mobile user**, I may see **full-screen native ad slots** (`ShortsNativeAd`) interleaved in the reel list only after **retention gates**: the user has advanced past a **five-reel** threshold and the session has been active for at least **20 seconds** (`SHORTS_ADS_SESSION_DELAY_MS`). Ads are capped (**`MAX_SHORTS_ADS`** inserts in code) and inserted every **`SHORTS_ADS_AFTER_EVERY`** reels once enabled. **Expo web** builds do not run this ad path.
- **As a viewer**, if a video URL is stale, the client may **refetch the short by id** (`getPostById`) to obtain a fresh signed URL before playback fails permanently.

### Implementation notes (reference)

- **Reliability**: Shorts is wrapped in `ErrorBoundary` with `level="route"`.
- **Audio**: Coordinates with shared `audioManager` and `SongPlayer` where background music applies.
- **Real-time**: Listens for creation events (e.g. socket payload with `shortId`) to optionally refresh feed content when configured in code.
- **List height**: Item height accounts for tab bar (`TAB_BAR_HEIGHT`) so reels sit above the bottom navigation.
