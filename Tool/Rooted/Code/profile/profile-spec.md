...
## 12. Codebase alignment — navigation & TripScore UX (April 2026)

### Additional user stories (mobile)

- **As a profile owner**, I can open **TripScore** at `/tripscore/continents?userId=…` from the profile TripScore card to explore continents for that user.
- **As a profile owner**, I can open **all locations on a map** via `/map/all-locations?userId=…` from the travel map entry points implemented on the profile screen.
- **As a user**, I can open **followers / following** lists with `/followers?userId=…&type=followers|following`.
- **As a user**, I can open **notifications** from the profile shell via `/notifications`.
- **As a user**, I can drill into **my grid post** via `/user-posts/:userId?postId=…` and **my shorts** via `/user-shorts/:userId?shortId=…` (including deep-link style focus on one item).
- **As a user**, I can open the **saved-posts** browser at `/saved-posts?postId=…` from saved-content actions.
- **As a user**, I reach **Collections** and **Activity** from **Settings** (`frontend/app/settings/index.tsx` lists `/collections` and `/activity`) rather than only from the tab profile layout.

### Trip “count” heuristic (reference)

`frontend/app/(tabs)/profile.tsx` derives a **trip count** from dated locations using a gap threshold **`TRIP_GAP_DAYS = 7`**: locations more than seven days apart start a new trip. Document this when reporting “trips” in analytics or UI copy so it is not confused with TripScore visits.

### Reliability

- Profile stack uses **`ErrorBoundary`** where wired in the implementation file.

---

**Document Version**: 1.1  
**Last Updated**: April 2026 (§12 routes & Trip heuristic)  
