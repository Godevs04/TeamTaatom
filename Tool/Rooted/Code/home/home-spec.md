...
2. App checks for cached posts in AsyncStorage (`cachedPosts` payload with `timestamp`)
3. If cached and younger than **24 hours**, show cached posts immediately (optimistic shell), then still fetch fresh posts
...
**Caching**:
- First-page cache (`cachedPosts`) may show stale data for up to **24 hours** when used for instant display or offline fallback
- No fine-grained cache invalidation on peer actions (refresh replaces page 1 when successful)
...
## 12. Codebase alignment — features & user stories (April 2026)

The following behaviors are implemented in `frontend/app/(tabs)/home.tsx` and should be treated as first-class product behavior (some were missing from earlier sections of this spec).

### Additional user stories

- **As a returning user**, I can land on the Home feed scrolled to a specific post when the app is opened with a `postId` query parameter, so that deep links and notifications center the correct item.
- **As a mobile user**, I see **native ad slots** only after I have shown engagement (scrolled past the fifth feed item and been in the session long enough), so that new sessions are not immediately monetized.
- **As a user who lost network mid-action**, my **like intentions are persisted** and retried when possible (`flushPendingLikes` + pending storage), so likes are less likely to be lost if the app is killed before the API responds.
- **As an offline user**, I can still see the **last fetched feed** from cache for up to **24 hours** (not 5 minutes) when the first page fails due to network errors, with pagination disabled until back online.
- **As a user browsing on web (Expo web)**, I do **not** see interleaved native ad rows; the feed is posts-only on web builds.

### Implementation notes (reference)

- **Feed composition**: Post rows are de-duplicated by `_id` when merging pagination results. Interleaved items use `NativeAdCard` every `ADS_AFTER_EVERY` (6) posts after gated conditions; see `feedData` / `isAdItem` types.
- **View analytics**: `trackPostView` is debounced per post (minimum interval **2 seconds**) and uses `viewabilityConfig` (50% visible, 200ms minimum view time).
- **Performance**: Images for post cards may omit heavy decoding when the row is more than **2** positions from the visible index (`shouldRenderImage`); `removeClippedSubviews`, `windowSize`, and batch sizes are tuned for scroll performance.
- **Reliability**: The feed screen is wrapped in `ErrorBoundary` with `level="route"`.
- **Unread chat badge**: Header badge counts unseen messages by normalizing participant and sender IDs (including Buffer-shaped IDs) and calling `GET /api/v1/chat` with the user token. While Home is focused, unseen counts **poll every 10 seconds** in addition to focus/refresh triggers.
- **Leaving Home**: `audioManager.stopAll()` runs when the screen blurs so feed-related audio does not continue in the background.

---

**Document Version**: 1.1  
**Last Updated**: April 2026 (§12 codebase alignment)  
