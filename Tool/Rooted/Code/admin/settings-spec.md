...
- **Route index**: See `README.md` § *SuperAdmin routes & permissions*

---

## 15. Implementation note — Settings vs System (April 2026)

**Database-backed product settings** (SMTP, security knobs, maintenance toggles, etc.) are edited on **`/settings`** via `superAdmin/src/pages/Settings.jsx` and patch endpoints on `/api/superadmin/settings` as described above.

**Live infrastructure / health / caches / DB collection stats** are surfaced on a **separate** screen **`/system`** (`superAdmin/src/pages/System.jsx`) with routes such as `/api/v1/superadmin/system/health` (and related system endpoints in `enhancedSuperAdminRoutes`). Do not conflate the “System” *section* inside the Settings form with the **System** *sidebar* entry—they are different surfaces in the current codebase.

See `README.md` § *SuperAdmin routes & permissions* for the full route table.
