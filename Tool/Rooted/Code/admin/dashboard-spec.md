...
- **Route index**: See `README.md` § *SuperAdmin routes & permissions* for `/system`, `/support-inbox`, and other global routes

---

## 15. Global SuperAdmin context (April 2026)

Screens such as **System** (`/system`), **Support Inbox** (`/support-inbox`), **Profile** (`/profile`), and the **dev-only** `/test` route are registered in `superAdmin/src/App.jsx` but are not re-specified in this dashboard document. Permission gating for sidebar items matches `superAdmin/src/components/Sidebar.jsx`. **Scheduled downtime** scheduling currently lives inside `FeatureFlags.jsx` (see `feature-flags-spec.md` §15), not as a standalone routed page.
