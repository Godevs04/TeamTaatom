...
## 15. Codebase note — scheduled downtime (April 2026)

Besides feature-flag CRUD, the `FeatureFlags.jsx` implementation includes **scheduled maintenance / downtime** scheduling and listing, backed by SuperAdmin endpoints under `/api/v1/superadmin/scheduled-downtimes` (see `superAdmin/src/pages/ScheduledDowntime.jsx` for a standalone UI variant that is **not** wired in `App.jsx` today).

### User story (ops)

- **As a SuperAdmin operator**, I can schedule maintenance windows and notify users (per the UI copy in `FeatureFlags.jsx`) so the mobile/web clients can prepare for downtime.

When updating this spec, keep §15 in sync with whichever UI surface is canonical (embedded vs future dedicated route).
