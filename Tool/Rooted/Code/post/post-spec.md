...
## 12. Codebase alignment — composition & compliance (April 2026)

### Additional user stories

- **As a creator**, I can open an **image edit / filter modal** (`ImageEditModal`, `ImageFilterType`) before upload to adjust the look of photos in the composer.
- **As a creator**, I must confirm **copyright / rights** via `CopyrightConfirmationModal` where the flow requires it, so uploads are not sent without explicit acknowledgment.
- **As a creator**, my caption goes through **validation and sanitization** (`validateAndSanitizeCaption` / related utilities) to reduce abusive or malformed text reaching the API.
- **As a creator**, the compose screen uses **Formik** with shared `postSchema` / `shortSchema` validation from `frontend/utils/validation` for consistent client-side checks alongside upload logic.

### Implementation notes (reference)

- Upload helpers include **progress-tracked** variants (`createPostWithProgress`, `createShortWithProgress`) used for long-running media uploads.
- Heavy media work may use **`expo-file-system/legacy`** and `expo-image-manipulator` / `expo-video-thumbnails` as implemented in `frontend/app/(tabs)/post.tsx`.
- The screen participates in **analytics** via `trackFeatureUsage` where instrumented.
- **ErrorBoundary** wraps the route-level compose experience where configured in code.

---

**Document Version**: 1.1  
**Last Updated**: April 2026 (§12 compose implementation)  
