...
const TRACKING_HEALTH_CHECK_INTERVAL = 15000;
const TRACKING_STALE_THRESHOLD = 30000;
...
const validateGPSPoint = (
  ...
  // Rule 3: speed > 150 km/h (41.67 m/s) check
  const speedMps = coord.speed;
  if (typeof speedMps === 'number' && speedMps > 41.67) {
    diagnostics.rejectedJump += 1;
    return false;
  }
  ...
  if (lastCoord) {
    ...
    const timeDiffSeconds = Math.max(0.1, (coord.timestamp - lastCoord.timestamp) / 1000);
    const calculatedSpeedMps = dist / timeDiffSeconds;

    // Rule 2: speed < 1 km/h (0.28 m/s) and jump > 20m check
    const effectiveSpeed = typeof speedMps === 'number' && speedMps >= 0 ? speedMps : calculatedSpeedMps;
    if (effectiveSpeed < 0.28 && dist > 20) {
      diagnostics.rejectedJump += 1;
      return false;
    }

    // Rule 4: GPS sudden jump filter (>40m in less than 2 seconds)
    if (dist > 40 && timeDiffSeconds < 2) {
      diagnostics.rejectedJump += 1;
      return false;
    }
  }

  diagnostics.acceptedPoints += 1;
  return true;
};
  ...
  TaskManager.defineTask(BG_LOCATION_TASK, async (taskBody: any) => {
    ...
    try {
      ...
      try {
        ...
        const existingAsync = Array.isArray(parsed) ? parsed : [];
        await AsyncStorage.setItem(queueKey, JSON.stringify([...existingAsync, ...valid]));
      } catch (e) {
        // ignore async queue write error
      }
    } catch {
...
export function useJourneyTracking(): UseJourneyTrackingReturn {
  ...
  const diagnosticsRef = useRef<GPSDiagnostics>({
    acceptedPoints: 0,
    rejectedAccuracy: 0,
    rejectedJump: 0,
    totalAccuracySum: 0,
  ...
  const queuedPolylineCoordsRef = useRef<Coordinate[]>([]);
  const polylineUpdateTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  ...
  const flushPolylineQueue = useCallback(() => {
    if (queuedPolylineCoordsRef.current.length === 0) return;
    const coordsToAppend = queuedPolylineCoordsRef.current.splice(0);
    setPolyline((prev) => {
      const next = [...prev, ...coordsToAppend];
      console.log("Journey Points:", next.length);
      return next;
    });
  }, []);
  ...
  // distanceInterval is 3 meters to filter small noise; we still gate polyline appends
  // on getAdaptiveMinDistance to keep the path clean.
  const startLocationWatcher = useCallback(async () => {
    if (Platform.OS === 'web') return;
    if (locationWatcherRef.current) return; // already running on this instance

    const initialTimeInterval = 2000;
    const initialDistanceInterval = 3;
    const initialAccuracy = Location.Accuracy.BestForNavigation;
  ...
  const adjustWatcherAdaptiveThrottling = useCallback((speed: number) => {
    let targetTimeInterval = 2000;
    let targetDistanceInterval = 3;
    let targetAccuracy = Location.Accuracy.BestForNavigation;

    if (speed < WALKING_SPEED_MPS) {
      // Walking / Stationary
      targetTimeInterval = 2000;
      targetDistanceInterval = 3;
      targetAccuracy = Location.Accuracy.BestForNavigation;
    } else if (speed < DRIVING_SPEED_MPS) {
      // Cycling
      targetTimeInterval = 1500;
      targetDistanceInterval = 5;
      targetAccuracy = Location.Accuracy.BestForNavigation;
    } else {
      // Driving
      targetTimeInterval = 1000;
  ...
  useEffect(() => {
    return () => {
      ...
      if (polylineUpdateTimeoutRef.current) clearTimeout(polylineUpdateTimeoutRef.current);
    };
  }, []);
  ...
  const syncPendingCompletions = useCallback(async () => {
    try {
      ...
      for (const item of queue) {
        const { journeyId, snapToRoads, endCoords } = item;
        const previousJourneyId = journeyIdRef.current;
        try {
          logger.debug(`[Journey] Retrying completion for journey: ${journeyId}`);
          journeyIdRef.current = journeyId;
          await drainBackgroundQueue(journeyId);
          const drainedCoords = batchCoordinatesRef.current.splice(0);

          const persistedKey = PENDING_COORDS_KEY_PREFIX + journeyId;
          const persistedRaw = await AsyncStorage.getItem(persistedKey);
          let persistedCoords: Coordinate[] = [];
          if (persistedRaw) {
            const parsed = JSON.parse(persistedRaw);
            if (Array.isArray(parsed)) {
              persistedCoords = parsed.filter((c: any) =>
                c &&
                typeof c === 'object' &&
                typeof c.latitude === 'number' &&
                typeof c.longitude === 'number' &&
          ...
          const coordsToFlush = [...persistedCoords, ...drainedCoords];
          if (coordsToFlush.length > 0) {
            await AsyncStorage.setItem(persistedKey, JSON.stringify(coordsToFlush)).catch(() => {});
            await updateJourneyLocation(journeyId, coordsToFlush);
          ...
          if (journeyIdRef.current === journeyId) {
            journeyIdRef.current = previousJourneyId;
          }
        }
      }
      ...
      try {
        ...
        if (storedJourneyId) {
          ...
          try {
            ...
            if (localStateRaw) {
              const localState = JSON.parse(localStateRaw);
              if (Array.isArray(localState?.polyline)) {
                recoveryPolyline = localState.polyline;
              }
            }
          } catch {
            // ignore local recovery parse failures
          }

          if (recoveryPolyline.length === 0 && fetchedJourney) {
            recoveryPolyline = normalizeJourneyPolyline(fetchedJourney);
          }

          if (recoveryPolyline.length > 0) {
            polylineRef.current = recoveryPolyline;
            setPolyline(recoveryPolyline);
            lastCoordinateRef.current = recoveryPolyline[recoveryPolyline.length - 1];
          }
          ...
          await drainBackgroundQueue(storedJourneyId);

          let pendingCoordUploadFailed = false;
          ...
          try {
            ...
            pendingCoordUploadFailed = true;
            logger.warn('[Journey] Failed to upload pending coords on auto-save:', e);
          }

          if (batchCoordinatesRef.current.length > 0) {
            const recoveredBatch = batchCoordinatesRef.current.splice(0);
            try {
              await updateJourneyLocation(storedJourneyId, recoveredBatch);
              clearPersistedCoords(storedJourneyId);
            } catch (e) {
              pendingCoordUploadFailed = true;
              batchCoordinatesRef.current.unshift(...recoveredBatch);
              persistPendingCoords();
              logger.warn('[Journey] Failed to upload recovered bg coords on auto-save:', e);
            }
          }

          const recoveredEndCoord = lastCoordinateRef.current
            ? {
                lat: lastCoordinateRef.current.latitude,
                lng: lastCoordinateRef.current.longitude,
              }
            : undefined;

          // Complete the journey on the backend
          try {
            if (pendingCoordUploadFailed) {
              throw new Error('Pending coordinates could not be uploaded before completion');
            }
            await completeJourney(storedJourneyId, { snapToRoads: true, endCoords: recoveredEndCoord });
            ...
            try {
              ...
              if (!queue.some((item: any) => item.journeyId === storedJourneyId)) {
                queue.push({ journeyId: storedJourneyId, snapToRoads: true, endCoords: recoveredEndCoord, timestamp: Date.now() });
  ...
  useEffect(() => {
    if (isTracking && !isPaused) {
      lastGpsUpdateAtRef.current = lastGpsUpdateAtRef.current || Date.now();
  ...
  useEffect(() => {
    if (!isTracking || isPaused || Platform.OS === 'web') return;

    healthCheckTimerRef.current = setInterval(async () => {
      if (!isMountedRef.current || !isTrackingRef.current || isPausedRef.current) return;
      const lastUpdateAt = lastGpsUpdateAtRef.current;
      if (!lastUpdateAt || Date.now() - lastUpdateAt <= TRACKING_STALE_THRESHOLD) return;

      logger.warn('[Journey] Tracking health stale; restarting location updates');
      lastGpsUpdateAtRef.current = Date.now();
      try {
        ...
        logger.warn('[Journey] Tracking health recovery failed:', e);
      }
    }, TRACKING_HEALTH_CHECK_INTERVAL);
  ...
  }, [isTracking, isPaused, startBackgroundUpdates, startLocationWatcher]);
    ...
    async (title?: string) => {
      ...
      try {
        ...
        diagnosticsRef.current = {
          ...
          rejectedAccuracy: 0,
          rejectedJump: 0,
          totalAccuracySum: location.coords.accuracy || 0,
          ...
          if ((err.message && err.message.includes('not active or paused')) || (err.response?.data?.message && err.response.data.message.includes('not active or paused'))) {
            ...
            try {
              ...
              if (!queue.some((item: any) => item.journeyId === completingJourneyId)) {
                queue.push({ journeyId: completingJourneyId, snapToRoads, endCoords: explicitEndCoords, timestamp: Date.now() });
                await AsyncStorage.setItem('@pending_journey_completions', JSON.stringify(queue));
  ...
  watcherCallbackRef.current = (location: Location.LocationObject) => {
    ...
    // Construct raw coordinate for filtering
    ...
    // Validate point BEFORE Kalman Filter and insertion
    const isValid = validateGPSPoint(rawCoord, lastCoordinateRef.current, diagnosticsRef.current);
    if (!isValid) {
      console.log("[GPS Diagnostics] Point rejected:", {
        accuracy: rawCoord.accuracy,
        speed: rawCoord.speed,
        acceptedPoints: diagnosticsRef.current.acceptedPoints,
        rejectedAccuracy: diagnosticsRef.current.rejectedAccuracy,
        rejectedJump: diagnosticsRef.current.rejectedJump,
        avgAccuracy: diagnosticsRef.current.totalPointsReceived > 0 
          ? (diagnosticsRef.current.totalAccuracySum / diagnosticsRef.current.totalPointsReceived).toFixed(1) + 'm' 
          : '0m',
        gpsSignalQuality: rawCoord.accuracy < 5 ? 'Excellent' : rawCoord.accuracy <= 10 ? 'Good' : rawCoord.accuracy <= 20 ? 'Fair' : 'Poor'
      });
      return;
    }

    // Apply Kalman Filter smoothing to validated coordinates
    if (!kalmanFilterRef.current) {
    ...
    console.log("[GPS Diagnostics] Point accepted:", {
      acceptedPoints: diagnosticsRef.current.acceptedPoints,
      rejectedAccuracy: diagnosticsRef.current.rejectedAccuracy,
      rejectedJump: diagnosticsRef.current.rejectedJump,
      avgAccuracy: (diagnosticsRef.current.totalAccuracySum / diagnosticsRef.current.totalPointsReceived).toFixed(1) + 'm',
      gpsSignalQuality: coord.accuracy < 5 ? 'Excellent' : coord.accuracy <= 10 ? 'Good' : coord.accuracy <= 20 ? 'Fair' : 'Poor'
    });
    ...
    if (lastCoordinateRef.current) {
      ...
      if (pendingSegmentBreakRef.current) {
        ...
        queuePolylineUpdate(segmentStart);
