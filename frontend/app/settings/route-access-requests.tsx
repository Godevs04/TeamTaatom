...
import { getRouteAccessRequests, approveRouteAccess, rejectRouteAccess } from '../../services/profile';
...
const logger = createLogger('RouteAccessRequestsScreen');

export default function RouteAccessRequestsScreen() {
  ...
  const loadRequests = async () => {
    try {
      const response = await getRouteAccessRequests();
      setRequests(response.requests || []);
    } catch (error) {
      logger.error('Error loading route access requests', error);
      showError('Failed to load route access requests');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleApprove = async (requestId: string, userName: string) => {
    try {
      await approveRouteAccess(requestId);
      setRequests(prev => prev.filter(req => req.user._id !== requestId));
      showSuccess(`${userName} can now view your completed traveling routes!`);
    } catch (error: any) {
      logger.error('Error approving route access request', error);
  ...
  const handleReject = async (requestId: string, userName: string) => {
    try {
      await rejectRouteAccess(requestId);
      setRequests(prev => prev.filter(req => req.user._id !== requestId));
      showSuccess(`Route access request from ${userName} has been declined`);
    } catch (error) {
      logger.error('Error rejecting route access request', error);
  ...
  if (loading) {
    return (
      <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
        <NavBar title="Route Access Requests" showBack={true} onBack={() => router.back()} />
      ...
      >
        {requests.length === 0 ? (
          <View style={styles.emptyContainer}>
            <Ionicons name="git-pull-request-outline" size={64} color={theme.colors.textSecondary} />
                ...
                <View style={styles.requestContent}>
                  ...
                  <View style={styles.actionButtons}>
                    <TouchableOpacity
                      ...
                      onPress={() => handleReject(request.user._id, request.user.fullName)}
                    >
                      <Ionicons name="close" size={16} color="white" />
                    </TouchableOpacity>
                    <TouchableOpacity
                      style={[styles.actionButton, styles.approveButton]}
                      onPress={() => handleApprove(request.user._id, request.user.fullName)}
                    >
