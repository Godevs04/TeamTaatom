...
/** Builds the id an option is referenced by, shared with the field's aria-activedescendant. */
export function mentionOptionId(listboxId: string, userId: string): string {
  return `${listboxId}-option-${userId}`;
}
 ...
 *
 * `highlightedIndex` drives both the visual highlight and `aria-selected`; the
 * field itself (not this list) owns focus, so the field's `aria-activedescendant`
 * must point at `mentionOptionId(listboxId, user._id)` for the highlighted user —
 * that's what lets a screen reader announce the right option while the caret
    ...
    >
      {users.map((u, index) => {
        ...
        return (
          <li
            key={u._id}
            id={mentionOptionId(listboxId, u._id)}
            role="option"
          ...
          >
            <button
              ...
              className={cn(
                "flex w-full items-center gap-3 px-3 py-2 text-left transition-colors hover:bg-slate-50 dark:hover:bg-zinc-800",
                highlighted && "bg-slate-50 dark:bg-zinc-800"
              )}
            >
