 ...
 */
export function useMentionAutocomplete(enabled = true) {
  ...
  /**
   * Text and cursor to apply for the picked user, or null if the mention is no
   * longer active. Callers set their own state and then restore the caret.
   */
  const select = React.useCallback(
    (user: MentionUser): { text: string; cursor: number } | null => {
  ...
  return { suggestions, sync, dismiss, select };
}
