  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-6">
      <div className="flex items-center gap-4 text-sm">
        ...
        <span className="font-medium text-slate-900">{data.country}</span>
      </div>

      <div className="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-premium">
        <h1 className="text-xl font-semibold text-slate-900">{data.country}</h1>
      ...
      {data.locations.length > 0 && (
        <Link
          ...
          className="flex items-center justify-between rounded-2xl border border-slate-200/80 bg-white p-4 shadow-premium transition hover:border-sky-200 hover:bg-slate-50"
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="font-semibold text-slate-900">Explore on Map</p>
              <p className="text-sm text-slate-500">View {data.country} locations</p>
            </div>
          </div>
      ...
      <section>
        ...
        {data.locations.length === 0 ? (
          <Card className="rounded-2xl border border-slate-200/80 p-8 text-center shadow-premium">
            <p className="text-sm text-slate-500">No locations in this country.</p>
          </Card>
        ) : (
                ...
                <div className="rounded-xl border border-slate-200/80 bg-white p-4 shadow-premium">
                  <div className="flex gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {loc.caption ? (
                        <p className="mt-1 line-clamp-2 text-sm text-slate-600">{loc.caption}</p>
                      ) : null}
                    </div>
                  </div>
                </div>
