  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-4 rounded-3xl border border-slate-200/80 bg-white p-6 shadow-premium dark:border-zinc-800/80 dark:bg-zinc-900/90 md:flex-row md:items-center md:justify-between md:p-8">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-zinc-50 md:text-3xl">Notifications</h1>
          <p className="mt-1 text-sm text-slate-500 dark:text-zinc-400">Activity and updates</p>
        </div>
        {unreadCount > 0 && (
          <Button
            variant="outline"
            size="sm"
            className="rounded-xl border-slate-200/80 dark:border-zinc-700"
      ...
      {isLoading ? (
        <div className="space-y-4">
          {[1, 2, 3, 4, 5].map((i) => (
            <div
              key={i}
              className="flex gap-4 rounded-2xl border border-slate-200/80 bg-white p-4 shadow-premium dark:border-zinc-800/80 dark:bg-zinc-900/90"
            >
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white p-12 text-center shadow-premium dark:border-zinc-800/80 dark:bg-zinc-900/90">
          <p className="text-slate-600 dark:text-zinc-400">Failed to load notifications.</p>
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white p-16 text-center shadow-premium dark:border-zinc-800/80 dark:bg-zinc-900/90">
          ...
          <h3 className="mt-6 text-lg font-semibold text-slate-900 dark:text-zinc-50">No notifications yet</h3>
          <p className="mt-2 text-[15px] text-slate-500 dark:text-zinc-400">When you get likes, comments or new followers, they&apos;ll show here.</p>
          <div className="mt-6">
        ...
        <div className="space-y-6">
          {sections.map((section) => (
            <section key={section.title}>
              <h2 className="mb-3 text-xs font-semibold uppercase tracking-widest text-slate-500 dark:text-zinc-400">{section.title}</h2>
              <div className="space-y-2">
                {section.data.map((n) => {
                  ...
                  const content = (
                    <div
                      className={`flex gap-4 rounded-2xl border border-slate-200/80 bg-white p-4 shadow-premium transition-shadow hover:shadow-premium-hover dark:border-zinc-800/80 dark:bg-zinc-900/90 dark:hover:shadow-premium-hover ${
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-[15px] font-medium text-slate-900 dark:text-zinc-50">{getNotificationLabel(n)}</p>
                        <p className="text-xs text-slate-500 dark:text-zinc-400">{formatTime(n.createdAt)}</p>
                      </div>
                    </div>
                  );
                  if (link?.href) {
